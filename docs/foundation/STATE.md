...
| `user_overall_progress` | Aggregated user metrics. Recomputed server-side via the `recompute_overall_progress` trigger function (formula **amended 2026-06-10, C13**: 50% lessons + 50% games split across **2 games** — opposites + dictation, 25% each; image_match dropped, see [2026-06-10_c13_drop_image_match_from_overall.sql](../../services/api/migrations/2026-06-10_c13_drop_image_match_from_overall.sql); subgame complete = ≥80% items correct lifetime). Client reads via `useOverallProgress()`; the Profile screen surfaces `progress_pct`. | `user_id` (PK), `total_score`, `progress_pct`, `recomputed_at` | [services/api/overall.ts](../../services/api/overall.ts) |
