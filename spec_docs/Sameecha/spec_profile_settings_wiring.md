...
5. **No new state library.** TanStack Query for server reads/writes; Zustand for device-only prefs. ~~Time pickers use the standard RN `DateTimePicker` (already a transitive dep via Expo SDK 54, not Expo Notifications).~~ *(Amended 2026-07-13, owner-directed: reminder time is now picked on the shared custom `TimeWheelPicker` (`components/ui/TimeWheelPicker.tsx`, quarter-hour steps) everywhere — RemindersSheet and the onboarding reminder step. The native `DateTimePicker` and quick-pick preset chips were removed from RemindersSheet; the wheel edits a pending value and "Save reminder" commits it. See spec_onboarding_reminder_step.md §4 amendment.)*
